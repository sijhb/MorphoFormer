...
# 采用了多个扩张尺度（dilation=[1, 2, 3]）来进行局部注意力计算，使得模型可以处理不同大小的局部信息。
class MultiDilatelocalAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # 修复：避免inplace操作，使用中间变量
        x_permuted = x.permute(0, 3, 1, 2)  # B, C, H, W
        
        # 计算qkv，每一步都确保创建新张量
        qkv = self.qkv(x_permuted)
        qkv_reshaped = qkv.reshape(B, 3, self.num_dilation, C//self.num_dilation, H, W)
        qkv_permuted = qkv_reshaped.permute(2, 1, 0, 3, 4, 5)
        
        # 处理输入x
        x_reshaped = x_permuted.reshape(B, self.num_dilation, C//self.num_dilation, H, W)
        x_final = x_reshaped.permute(1, 0, 3, 4, 2)
        ...
        for i in range(self.num_dilation):
            ...
            k = qkv_permuted[i][1] 
            v = qkv_permuted[i][2]
            result = self.dilate_attention[i](q, k, v)
            results.append(result)
        
        # 使用stack组合结果
        x_stacked = torch.stack(results, dim=0)
        x_combined = x_stacked.permute(1, 2, 3, 0, 4).reshape(B, H, W, C)
...
class DilateBlock(nn.Module):
    ...
    def forward(self, x):
        if self.cpe_per_block:
            # 修复：避免可能的inplace操作
            pos_embed_out = self.pos_embed(x)
            x = x + pos_embed_out
        ...
        # 第一个残差连接
        residual1 = x_permuted
        x_norm1 = self.norm1(x_permuted)
        x_attn = self.attn(x_norm1)
        x_drop1 = self.drop_path(x_attn)
        x_out1 = residual1 + x_drop1  # 非inplace加法
        ...
        x_out2 = residual2 + x_drop2  # 非inplace加法
        
        x_final = x_out2.permute(0, 3, 1, 2)
        return x_final
...
# 型的输入层，它将图像分割成小块并进行嵌入（通常为卷积操作），将每个小块转化为特征表示。
...
# 下采样层，主要作用是通过卷积或池化操作将特征图的空间维度缩小，逐步提取更高层次的特征。
...
# Dilatestage 和 Globalstage 是整个模型的组成单元，每一层包含多个 Transformer 块（DilateBlock 或 GlobalBlock）。
...
# 模型的核心架构。它包括多个阶段，每个阶段可以选择使用 Dilatestage 或 Globalstage，这样能够同时捕捉局部和全局特征。
# 该模型的输入为图像数据，通过 PatchEmbed 层将图像转换为一系列小块的嵌入表示，然后通过多个阶段进行特征提取和下采样。
# 最终，模型输出的特征图可以用于分类或其他任务。
class Dilateformer(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=7, embed_dim=96,
